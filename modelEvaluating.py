...
def evalPartitions(trainingPartitions_sorted, trainedModels_properties,
    ...
    #modelsEval_properties = pd.DataFrame(columns=columnNames) #deprecated pandas release
    modelsEval_properties = []
    ...
    if storeModels:
        ...
        for prtn in np.arange(len(trainingPartitions_sorted)):
            ...
            filename = glob.glob(workdir + 'models/models_{0}/model*_Partition_{1}*.pkl'.format(expName, prtn))[0]
            models[modelID] = pickle.load(open(filename, 'rb'))
    ...
    for partition in np.arange(len(trainingPartitions_sorted)):
        ...
        #modelsEval_properties = modelsEval_properties._append(addProperties) #deprecated pandas release
        modelsEval_properties.append(addProperties)
    modelsEval_properties = pd.concat(modelsEval_properties)
...
def assembleCV_results(workDir, modelPerformances_dirs, numFolds, expName):
    ...
    precision_combinedWeighted_kCV, recall_combinedWeighted_kCV, _ = precision_recall_curve(labels_acrossFolds,
                                                                                            combinedWeighted_probsPos_kCV)
    os.makedirs(workDir + 'modelPerformance/modelsPerformance_ppiPrediction_performanceAnalysis-5CV_{0}'.format(expName), exist_ok=True)
    cvPerformance_path = \
...
def plot_kCV_prr(workDir, modelPerformances_dirs, numFolds, expName):
    ...
    precAcc, recallAcc, tAcc = precision_recall_curve(labels_acrossFolds, unitAccuracy_probsPos_acrossFolds)
    #labAcc = '{0}CV AUC={1:.4f}'.format(numFolds, auc(recallAcc, precAcc))
    #plt.step(recallAcc, precAcc, label=labAcc, lw=2)

    precWeighted, recallWeighted, tWeighted = precision_recall_curve(labels_acrossFolds, combinedWeighted_probsPos_acrossFolds)
    ...
    f1Scores = (2 * recallWeighted * precWeighted) / (recallWeighted + precWeighted)
    pickle.dump({'f1Scores': f1Scores,
                 'recall': recallWeighted,
                 'precision': precisionWeighted,
                 'threshold': tWeighted}, open(cvPerformance_path + 'prr_results.pkl', 'wb'))
    print('F1-optimal threshold: {0}'.format(tWeighted[np.argmax(f1Scores)]))
...
def plotAnalysis(workdir, expName, precRecall_pairSets):
    ...
    for precRecall_pair in precRecall_pairSets:
        ...
        plt.plot(precRecall_pair[0], precRecall_pair[1],
                 label=precRecall_pair[2] + ', AUC: {0: .2f}'.format(precRecall_auc))
    plt.xlabel('Recall')
    plt.ylabel('Precision')
    plt.legend(bbox_to_anchor=(1, 1), loc='upper left', ncol=1)
    plt.savefig(workdir + 'modelPerformance/modelsPerformance_{0}/'.format(expName) + filename + '.png', bbox_inches='tight')

def makePrR_varDom(df, studies, return_prr=False):
  #df = {pairsFrozen, prob_studyA, prob_studyB, label}

  studyA, studyB = studies
  keepCols = ['pairsFrozen', studyA, studyB, 'label']

  dfComplete_eitherStudy = \
    df.loc[df[studyA].notnull() | df[studyB].notnull(), keepCols].copy()

  dfComplete_eitherStudy[studyA + ' Dom'] = dfComplete_eitherStudy[studyA]
  dfComplete_eitherStudy[studyB + ' Dom'] = dfComplete_eitherStudy[studyB]

  dfComplete_eitherStudy.loc[
      dfComplete_eitherStudy[studyA + ' Dom'].isna(), [studyA + ' Dom']] = \
      dfComplete_eitherStudy.loc[dfComplete_eitherStudy[studyA + ' Dom'].isna(),
                                studyB]
  dfComplete_eitherStudy.loc[
      dfComplete_eitherStudy[studyB + ' Dom'].isna(), [studyB + ' Dom']] = \
      dfComplete_eitherStudy.loc[dfComplete_eitherStudy[studyB + ' Dom'].isna(),
                                studyA]

  if return_prr:
    studyA_domPrr = \
      precision_recall_curve(dfComplete_eitherStudy['label'],
                             dfComplete_eitherStudy[studyA + ' Dom'])
    studyB_domPrr = \
      precision_recall_curve(dfComplete_eitherStudy['label'],
                             dfComplete_eitherStudy[studyB + ' Dom'])

    return dfComplete_eitherStudy, studyA_domPrr, studyB_domPrr

  return dfComplete_eitherStudy