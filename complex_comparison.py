...
class ComplexComparison(object):
    ...
    def __init__(self, gold_standard=[], clusters=[], exclusion_complexes=[], samples=10000, pseudocount=1, exact=False,
                 max_clique=None, remove_non_gold_standard_proteins=False, normalize_by_combinations=False):
    ...
    def clique_comparison_metric(self, force=False):
        ...
        for size in range(2, max_len + 1):
            ...
            if size > 4 and return_dict[size - 1]['precision'] == zero_w_pseudocount and return_dict[size - 1][
                'recall'] == zero_w_pseudocount and return_dict[size - 2]['precision'] == zero_w_pseudocount and \
    ...
    def clique_comparison_exact(self, clique_size):
        ...
        if len(self.get_exclusion_complexes()) > 0:
            raise Exclusion_Complexes_Exception(
                "ERROR: EXCLUSION COMPLEX functionality is not implemented for exact method (def clique_comparison_exact), use estimated (def clique_comparison)")

        # kdrew: only get clusters that are larger than or equal to the clique size
        clusters = [clust & self.get_gold_standard_proteins() for clust in self.get_clusters() if
                    len(clust & self.get_gold_standard_proteins()) >= clique_size]

        for clust in clusters:
            is_positive_list = [np.max(map(set(group).issubset, self.get_gold_standard())) for group in
                                it.combinations(clust, clique_size)]
        ...
        for gs_clust in self.get_gold_standard():
            is_positive_list = [np.max(map(set(gs_group).issubset, clusters)) for gs_group in
                                it.combinations(gs_clust, clique_size)]
    ...
    def clique_comparison(self, clique_size):
        ...
        for random_clique in random_cliques:
            ...
            if np.max(map(random_clique.issubset, self.get_gold_standard())):
                ...
                if len(self.get_exclusion_complexes()) > 0 and np.max(
                        map(random_clique.issubset, self.get_exclusion_complexes())):
...
def main():
    ...
    parser.add_argument("--remove_non_gold_standard_proteins", action="store_true",
                        dest="remove_non_gold_standard_proteins", required=False, default=True,
                        help="Flag to remove proteins from clusters that are not in the gold standard, default=True")
    parser.add_argument("--normalize_by_combinations", action="store_true", dest="normalize_by_combinations",
    ...
    parser.add_argument("--pseudocount", action="store", type=float, dest="pseudocount", required=False,
                        default=0.00001,
    ...
    sensitivity, ppv, acc, mmr, pwmmr, pwmmr_hmean, precision_measure, recall_measure, precision_recall_product,
    clique_pr_mean, clique_re_mean, clique_f1grand, clique_weighted_pr_mean, clique_weighted_re_mean,
