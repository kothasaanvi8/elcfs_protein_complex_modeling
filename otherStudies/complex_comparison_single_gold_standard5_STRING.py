...
class ComplexComparison(object):
    ...
    def __init__(self, clustersFilename=[], exclusion_complexes=[], samples=10000, pseudocount=1,
                 exact=False, max_clique=None, remove_non_gold_standard_proteins=True,
        ...
        gold_standard = []
        gold_file = open(sys.path[0] + 'Primary Research/proteinPairs_complexMaps/sourceData/string/complexes/stringComplexes_geneIDmappings_unique.txt', 'rb')
        for line in gold_file.readlines():
            gold_standard.append(line.split())
        gold_file.close()
        ...
        clusters = []
        clustersFile = open(clustersFilename, "rb")
        ...
        # debugging issues with weighting
        self.WeightedRandomGeneratorstruct1 = None
        self.WeightedRandomGeneratorstruct2 = None
    ...
    # kdrew: method described in Yang et al.
    # BMC Medical Genomics Integrating PPI datasets with the PPI data from biomedical literature
    # ...for protein complex detection (2014)
    ...
    # kdrew: method from Brohee and Helden BMC Bioinformatics,
    # ...http://www.biomedcentral.com/content/pdf/1471-2105-7-488.pdf
    ...
    # kdrew: harmonic mean of f1-scores across clique sizes,
    # ...single score for comparison of cluster predictions to gold standard complexes
    ...
    def clique_comparison_metric(self, force=False):
        ...
        # kdrew: find the max cluster sizes of all clusters and gold standard complexes,
        # ...then find the min between the maxes
        # kdrew: I don't know if I like this because it will not report anything for predictions that
        # ...are considered false positives but are to large to be considered
        ...
        for size in range(2, max_len + 1):
            ...
            # kdrew: shortcut, if the last two precision and recall estimates are 0.0 then there is
            # ...exceedingly small chance are a larger clique size will have a precision/recall > 0.0
            ...
            # kdrew: if clique size is greater than the max size of the gold standard,
            # ...return precision 0.0 and recall 0.0
            # kdrew: (technically recall should be NA because there are no complexes left in the gold standard,
            # ...but I think it makes sense to reduce it to 0.0 here)
            ...
            if self.exact:
                ...
                ##GW made changes (if any weights exceed resource limits (i.e., inf), terminate loop and proceed
                try:
            ...
            # kdrew: calculate f1score (hmean of precision and recall) for each clique size,
            # hmean is undefined when any value is <= 0, force it to be 0.0
            f1score = 0.0
    ...
    def clique_comparison_exact(self, clique_size):
        ...
        for clust in clusters:
            ...
            if self.normalize_by_combinations:
                # kdrew: for each cluster weight true positives by the size of the cluster
                # ...divided by the number of cliques
                tp_curr = tp_curr * (1.0 * len(clust) / misc.comb(len(clust), clique_size))

            true_positives += tp_curr
            fp_curr = len(is_positive_list) - tp_curr
            if self.normalize_by_combinations:
                # kdrew: for each cluster weight false positives by the size of the cluster
                # ...divided by the number of cliques
        ...
        for gs_clust in self.get_gold_standard():
            ...
            if self.normalize_by_combinations:
                # kdrew: for each cluster weight gs true positives
                # by the size of the gold_standard cluster divided by the number of cliques
            ...
            if self.normalize_by_combinations:
                # kdrew: for each cluster weight false negatives
                # by the size of the gold_standard cluster divided by the number of cliques
    ...
    def clique_comparison(self, clique_size):
        ...
        self.WeightedRandomGeneratorstruct1 = weights_array  #added for debugging of weighting code
        ...
        for s in range(self.samples):
            ...
            # kdrew: if all proteins are outside of the gold standard,
            # ...move on (unlikely to ever get selected due to weighted sampling)
            ...
            # kdrew: for each random cluster clique, weight
            # ...by the size of the random cluster divided by the number of cliques
        ...
        # kdrew: NOTE: all individual cliques should be treated equally
        # ...but this might be overweighting cliques that are in multiple complexes, not sure how to fix yet
        # kdrew: this should be fixed now with the random_gs_cliques set, only evaluates a clique once
        self.WeightedRandomGeneratorstruct2 = [misc.comb(len(gs_clust), clique_size) for gs_clust in gs_clusters]  #added for debugging of weighting code
...
## (GW) made changes to accommodate circumstances in which weights exceed resource limits (e.g., inf)
...
def worker(arg):
    return arg.clique_comparison_metric_mean()


def main(cluster_filename, gold_standard_filename, plot_filename=None, remove_non_gold_standard_proteins=True,
         normalize_by_combinations=True, excluded_complexes=None, pseudocount=0.00001):
    # cluster_filename (cluster_predictions): Filename of cluster predictions, format 1 cluster/line, ids space separated
    # gold_standard_filename: Filename of gold standard complexes, format 1 complex/line, ids space separated
    # plot_filename: Filename for plotting histograms of metrics
    # remove_non_gold_standard_proteins: Flag to remove proteins from clusters not in the gold standard; default=True
    # normalize_by_combinations: Normalize clique precision recall by  # of combinations for each cluster; default=True
    # excluded_complexes: Filename of benchmark complexes to be excluded from false positive calculation; default=None
    # pseudocount: Set pseudocount for clique sampling, default=0.00001

    bypassInputArgs = \
        {"cluster_filename": cluster_filename, "gold_standard_filename": gold_standard_filename,
         "plot_filename": plot_filename, "random_seed": 42,
         "remove_non_gold_standard_proteins": remove_non_gold_standard_proteins,
         "normalize_by_combinations": normalize_by_combinations, "excluded_complexes": excluded_complexes,
         "pseudocount": pseudocount}
    args = Dict2Class(bypassInputArgs)
    ...
    excluded_complexes = []
    if args.excluded_complexes is not None:
    ...
    cplx_compare = ComplexComparison(gold_standard_complexes, predicted_clusters,
                                     remove_non_gold_standard_proteins=args.remove_non_gold_standard_proteins,
                                     exclusion_complexes=excluded_complexes,
                                     normalize_by_combinations=args.normalize_by_combinations,
    ...
    print("Sensitivity\tPPV\tACC\tMMR\tPWMMR\tMMR_PWMMR_hmean\tPrecision measure\tRecall measure\t"
          "Precision Recall product\tClique Precision Mean\tRecall Mean\tF-Grand K-Clique\t"
          "Clique Weighted Precision Mean\tWeighted Recall Mean\tClique Weighted hmean (F-weighted K-Clique)\n")
    print("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n"
          % (sensitivity, ppv, acc, mmr, pwmmr, pwmmr_hmean, precision_measure, recall_measure,
             precision_recall_product, clique_pr_mean, clique_re_mean, clique_f1grand, clique_weighted_pr_mean,
             clique_weighted_re_mean, clique_weighted_hmean))
