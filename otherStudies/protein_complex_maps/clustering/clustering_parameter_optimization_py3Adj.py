...
def cluster(input_network, gold_standard_filename, output_file):
    ...
    # random_seed: Sets random seed (int), default=None
    # bootstrap_iter: Number of bootstrap iterations (int, default=10)
    # bootstrap_fraction: Fraction of edges to sample for bootstrapping, default=0.5
    # clustone_jar: Location of cluster_one jar file, default= '/home/kdrew/programs/clusterone/cluster_one-1.0.jar'
    # clusterone_size: ClusterOne Size parameter sweep, default=2
    # clusterone_density: ClusterOne Density parameter sweep, default= [.1 .25 .3 .35]
    # clusterone_max_overlap: ClusterOne max-overlap parameter sweep, default= [0.5 0.75 0.9]
    # clusterone_seed_method: ClusterOne seed method parameter sweep
    # ...(nodes, cliques, unused_nodes, edges, default=nodes
    # ppi_fraction: Use top fraction for further clustering, default= [0.005 0.01 0.05 .1 .25 .5 .75 1.0]
    # ppi_threshold_score: Use ppis with score higher or equal to threshold score,
    # ...default= [1.0 0.9 0.8 0.7 0.6 0.5 0.4 0.3 0.2 0.1]
    # clixo_bin: default= '~/programs/mhk7-clixo_0.3-a2b23b0/clixo'
    # clixo_alpha: Clixo Alpha parameter, default=None
    # clixo_beta: Clixo Beta parameter, default=None
    # mcl_bin: Location of mcl binary, default='mcl'
    # mcl_inflation: MCL Inflation (-I) parameter, default = [None] (no 2-stage clustering), docs suggest = 1.2 - 5.0
    # cfinder_exe: Location of CFinder executable, default= '/home/kdrew/programs/CFinder-2.0.6--1448/CFinder_commandline64'
    # cfinder_license: Location of CFinder license, default= '/home/kdrew/programs/CFinder-2.0.6--1448/licence.txt'
    # cfinder_cliquesize: Cfinder clique size (-k) parameter, default=None (use CFinder's default setting, 3)
    # cfinder_timeout: finder timeout (-t) parameter, default=None (use CFinder's default setting, 10)
    # trim2threshold: Trim final clusters of subunits that do not have an edge that passes the threshold_score,
    # ...default=False")
    # twostep_combination: Combination of two step clustering, default= [clusterone, mcl],
    # ...options= [clusterone, mcl, cfinder, agglomod, clixo]
    # eval_metric: Evaluation metric used to determine best set of parameters (mmr, acc, sensitivity, ppv,
    # ...clique_precision_mean, clique_recall_mean), default=mmr
    # output_all: Output all clusterings, default=False
    # procs: Number processors to use (int), default=1
    # temp_dir: Where to store temporary files generated during processing, default=None
    # ...(defaults to OS level tmp), in memory suggestion= '/dev/shm/'
    # nodelete: (flag to or not to) delete temporary files
    ...
    bypassInputArgs = \
        {'input_network': input_network, 'gold_standard_filename': gold_standard_filename,
         ...
         "clusterone_size": [2, ],
         "clusterone_density": [.1, .25, .3, .35, ], "clusterone_max_overlap": [0.5, 0.75, 0.9],
         "clusterone_seed_method": ['nodes'], "ppi_fraction": [0.005, 0.01, 0.05, .1, .25, .5, .75, 1.0],
         "ppi_threshold_score": [1.0, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1],
         "clixo_bin": "/My Drive/Colab Notebooks/otherStudies/AIM_H_SAPIENS/clixo_0.3/clixo",
         "clixo_alpha": [None], "clixo_beta": [None], "mcl_bin": "mcl",
         "mcl_inflation": [None],
         "cfinder_exe": "/My Drive/Colab Notebooks/otherStudies/CFinder-2.0.6--1448/CFinder_commandline64",
         "cfinder_license": "/My Drive/Colab Notebooks/otherStudies/CFinder-2.0.6--1448/licence.txt",
         "cfinder_cliquesize": [None], "cfinder_timeout": [10],
         "trim2threshold": False, "twostep_combination": ['clusterone', 'mcl'], "eval_metric": "mmr",
         "output_all": False, "procs": 9, "temp_dir": "/My Drive/Colab Notebooks/otherStudies/temp/", "nodelete": False}
        ...
        for boot_cmp in bootstrap_cplx_cmp_metrics:
            print("bootstrapped: ii %s, size %s, density %s, overlap %s, seed_method %s, fraction %s, "
                  ...
                  % (ii, size, density, overlap, seed_method, fraction, threshold_score, inflation, clixo_alpha,
                     ...
                     metric_dict['precision_recall_product'], boot_cmp['clique_precision_mean'],
