...
def main(scoredPairs_filename, goldStandard_filename, outputFilename, tempDir):
    ...
    parser.add_argument("--clusterone", action="store", dest="clustone_jar", required=False,
                        default="/Users/wilkinsbusiness/Library/CloudStorage/GoogleDrive-gwilkins@andrew.cmu.edu/My Drive/Colab Notebooks/otherStudies/cluster_one-1.0.jar",
                        help="Location of cluster_one jar file, default= /Users/wilkinsbusiness/Library/CloudStorage/GoogleDrive-gwilkins@andrew.cmu.edu/My Drive/Colab Notebooks/otherStudies/cluster_one-1.0.jar")
    ...
    parser.add_argument("--clixo_bin", action="store", dest="clixo_bin", required=False,
                        default='/Users/wilkinsbusiness/Library/CloudStorage/GoogleDrive-gwilkins@andrew.cmu.edu/My Drive/Colab Notebooks/otherStudies/AIM_H_SAPIENS/clixo_0.3/clixo')
    ...
    parser.add_argument("--cfinder", action="store", dest="cfinder_exe", required=False,
                        default="/Users/wilkinsbusiness/Library/CloudStorage/GoogleDrive-gwilkins@andrew.cmu.edu/My Drive/Colab Notebooks/otherStudies/CFinder-2.0.6--1448/CFinder_commandline64",
                        help="Location of CFinder executable, default= /Users/wilkinsbusiness/Library/CloudStorage/GoogleDrive-gwilkins@andrew.cmu.edu/My Drive/Colab Notebooks/otherStudies/CFinder-2.0.6--1448/CFinder_commandline64")
    parser.add_argument("--cfinder_license", action="store", dest="cfinder_license", required=False,
                        default="/Users/wilkinsbusiness/Library/CloudStorage/GoogleDrive-gwilkins@andrew.cmu.edu/My Drive/Colab Notebooks/otherStudies/CFinder-2.0.6--1448/licence.txt",
                        help="Location of CFinder license, default= /Users/wilkinsbusiness/Library/CloudStorage/GoogleDrive-gwilkins@andrew.cmu.edu/My Drive/Colab Notebooks/otherStudies/CFinder-2.0.6--1448/licence.txt")
    ...
    args = \
        parser.parse_args(
            ["--input_network", scoredPairs_filename, "--gold_standard", goldStandard_filename,
             "--output_file", outputFilename, "--temp_dir", tempDir]
        )
    ...
    for cluster_prediction, ii in cluster_predictions:
        ...
        print "ii %s, size %s, density %s, overlap %s, seed_method %s, fraction %s, threshold_score %s, inflation %s, clixo_alpha %s, clixo_beta %s, cliquesize %s, timeout %s, twostep_combination: %s, trim2threshold: %s" % (
        ii, size, density, overlap, seed_method, fraction, threshold_score, inflation, clixo_alpha, clixo_beta,
        cliquesize, timeout, str(twostep_combination), str(trim2threshold))
        ...
        try:
            ...
            metric_dict['clique_hmean_mean_normalize_weighted'] = hmean([ccmm_normalize_weighted['precision_mean'], ccmm_normalize_weighted['recall_mean']])

        except Exception as e:
            print e
            continue

        print "ii %s, size %s, density %s, overlap %s, seed_method %s, fraction %s, threshold_score %s, inflation %s, clixo_alpha %s, clixo_beta %s,cliquesize %s, timeout %s, twostep_combination: %s, trim2threshold: %s, acc %s, sensitivity %s, ppv %s, mmr %s, precision_recall_product %s, clique_precision_mean %s, clique_recall_mean %s, clique_precision_mean_normalize %s, clique_recall_mean_normalize %s, clique_precision_mean_normalize_weighted %s, clique_recall_mean_normalize_weighted %s" % (
        ii, size, density, overlap, seed_method, fraction, threshold_score, inflation, clixo_alpha, clixo_beta,
        cliquesize, timeout, str(twostep_combination), str(trim2threshold), metric_dict['acc'],
        metric_dict['sensitivity'], metric_dict['ppv'], metric_dict['mmr'], metric_dict['precision_recall_product'],
        metric_dict['clique_precision_mean'], metric_dict['clique_recall_mean'],
        metric_dict['clique_precision_mean_normalize'], metric_dict['clique_recall_mean_normalize'],
        metric_dict['clique_precision_mean_normalize_weighted'], metric_dict['clique_recall_mean_normalize_weighted'])
        # sys.stdout.flush()
        ...
        for boot_cmp in bootstrap_cplx_cmp_metrics:
            ...
            ii, size, density, overlap, seed_method, fraction, threshold_score, inflation, clixo_alpha, clixo_beta,
            cliquesize, timeout, str(twostep_combination), str(trim2threshold), boot_cmp['acc'],
            boot_cmp['sensitivity'], boot_cmp['ppv'], boot_cmp['mmr'], boot_cmp['percent_ppi_recovered'],
            metric_dict['precision_recall_product'], boot_cmp['clique_precision_mean'], boot_cmp['clique_recall_mean'],
            metric_dict['clique_precision_mean_normalize'], metric_dict['clique_recall_mean_normalize'],
        ...
        if best_eval == None or best_eval < metric_dict[args.eval_metric]:
            ...
            print "best ii: %s size: %s density: %s overlap: %s seed_method: %s fraction: %s threshold_score: %s inflation: %s clixo_alpha: %s, clixo_beta: %s,cliquesize: %s timeout: %s twostep_combination: %s, trim2threshold: %s, numOfClusters: %s" % (
            best_ii, best_size, best_density, best_overlap, best_seed_method, best_fraction, best_threshold_score,
            best_inflation, best_clixo_alpha, best_clixo_beta, best_cliquesize, best_timeout,
            str(best_twostep_combination), str(trim2threshold), len(best_cluster_prediction))
...
def cluster_helper(parameter_dict):
    ...
    try:
        ...
        if twostep_combination[0] == 'clusterone':
            ...
            proc = sp.Popen(
                ['java', '-jar', args.clustone_jar, fileTemp.name, '-s', size, '-d', density, '--max-overlap', overlap,
                 '--seed-method', seed_method], stdout=sp.PIPE, stderr=sp.PIPE)
            ...
            proc = sp.Popen(
                [args.cfinder_exe, '-l', args.cfinder_license, '-i', fileTemp.name, '-o', dirTemp, '-k', cliquesize,
                 '-t', timeout], stdout=sp.PIPE, stderr=sp.PIPE)
            clust_out, err = proc.communicate()
            # print clust_out
            # print err
            print dirTemp
