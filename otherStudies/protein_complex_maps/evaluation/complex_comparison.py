...
class ComplexComparison(object):
    ...
    def __init__(self, gold_standard=[], clusters=[], exclusion_complexes=[], samples=10000, pseudocount=1,
    ...
    def clique_comparison_exact(self, clique_size):
        ...
        for clust in clusters:
            is_positive_list = [np.max(map(set(group).issubset, self.get_gold_standard()))
        ...
        for gs_clust in self.get_gold_standard():
            is_positive_list = [np.max(map(set(gs_group).issubset, clusters))
        ...
        for random_clique in random_cliques:
            ...
            if np.max(map(random_clique.issubset, self.get_gold_standard())):
                ...
                if len(self.get_exclusion_complexes()) > 0 and \
                        np.max(map(random_clique.issubset, self.get_exclusion_complexes())):
                    continue
                else:
                    if self.normalize_by_combinations:
