...
def getAllCliques_gwMods(pairsInfoFilename, pairsPredictionsFilename, cutoff, outputPATH):
    ...
    comparisonMatrix = {}
    outfilename = \
        outputPATH + pairsPredictionsFilename.split('/')[-1].split('.')[0] + '_candidates_complexesSize_2.txt'
    ...
    outfile.close()
    outfilename = \
        outputPATH + pairsPredictionsFilename.split('/')[-1].split('_features_')[0] + \
        '_candidates_complexesSize_2@' + str(cutoff) + '.gv'
    outfile = open(outfilename, 'w')
    ...
    outfilenameGraph = \
        outputPATH + pairsPredictionsFilename.split('/')[-1].split('_features_')[0] + \
        '_candidates_complexesSize_2@' + str(cutoff) + '.graph'
    ...
    for cliqueVerticesList in cliques:
        ...
        complexScore_mean = sum(scoresPerPair)/len(scoresPerPair)
        if complexSize in predictedComplexes:
            predictedComplexes[complexSize].append((complexID, complexScore, complexScore_mean))
        else:
            predictedComplexes[complexSize] = [(complexID, complexScore, complexScore_mean)]
    ...
    for complexSize, complexesSet in predictedComplexes.items():
        outfilename = outputPATH + outfilenamePrefix.split('/')[-1] + str(complexSize) + '.txt'
        outfile = open(outfilename, "w")
